...
import 'package:format_dio_logger/src/pretty_util.dart';
...
class FormatDioLogger extends Interceptor {
  ...
  void onRequest(RequestOptions options, RequestInterceptorHandler handler) {
    if (request) {
      List<String> buffer = [];
      buffer.addAll(_prettyUtil.prettyBoxHeader(header: 'Request ║ ${options.method} ╠', url: options.uri.toString()));
      if (requestHeader) {
  ...
  void onError(DioError err, ErrorInterceptorHandler handler) {
    if (error) {
      ...
      buffer.addAll(_prettyUtil.prettyMap({"type": err.type, "msg": err.error}));
